...
def plot_sentiment_series(videos, topics, channels, start_date=None, title=None):
    ...
    for i, topic in topics.reset_index().iterrows():
        ...
        ts = videos[topic_mask].set_index('Date').resample(resample_rule)['sentiment_score'].mean().interpolate()
        # sns.tsplot(ts, ts.index, color='#fcef99', linewidth=6, ax=ax)
        sns.lineplot(x=ts.index, y=ts, color='#fcef99', linewidth=6, ax=ax)

        # Plot a separate time-series for each channel
        for _, channel in channels.iterrows():
            channel_mask = topic_mask & (videos.User==channel.title)
            ts = videos[channel_mask].set_index('Date').resample(resample_rule)['sentiment_score'].mean().interpolate()
            if len(ts) > 1:
                sns.lineplot(x=ts.index, y=ts,color= channel['color'], linewidth=1, ax=ax)
